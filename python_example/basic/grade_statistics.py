...
def print_histogram(lst):
    ...
    for grade in lst:
        if grade == 100:  # Special case
            ...
            # Use // for integer divide to get a truncated int
            bins[grade//10] += 1
    ...
    for row in range(len(bins)):  # [0, 1, 2, ..., len(bins)-1]
        ...
        if row == 9:  # Special case
            # Formatted output (new style), no newline
            print('{:3d}-{:<3d}: '.format(90, 100), end='')
        else:
            print('{:3d}-{:<3d}: '.format(row*10, row*10+9),
                  end='')  # Formatted output, no newline
